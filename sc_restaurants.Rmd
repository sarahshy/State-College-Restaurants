...
ggplot(restaurants, aes(y = rating)) +
...
restaurants %>% filter(review_count <= 5) %>% select(name, review_count)
```
...
```{r}
restaurants %>% arrange(desc(review_count)) %>% select(name, review_count) %>% head(3)
```
...
```{r}
restaurants %>% arrange(desc(rating)) %>% select(name, rating, review_count) %>% head
```

The top-rated restaurant in State College is Cafe Alina with a bangin' 5 star rating. However, since Cafe Alina only has 2 reviews, the rating we see may not be representative of the true customer experience.

## Bottom Restaurants

The 6 most poorly rated restaurants are:
```{r}
restaurants %>% arrange(rating) %>% select(name, rating, review_count) %>% head
```
...
map_top_half <- ggmap(sc_map) +
  geom_point(data = restaurants, aes(x = longitude, y = latitude, col = top_half, group = name), alpha = 0.8) +
  labs(x = "", y = "", col = "Rating")
